...
pub static HOLDEM_MAP_DATA: LazyLock<Vec<u8>> = LazyLock::new(|| {
    include_bytes!(concat!(env!("OUT_DIR"), "/holdem-map-ranking-number.bin"))
        .to_vec()
});

pub static SHORTDECK_MAP_DATA: LazyLock<Vec<u8>> = LazyLock::new(|| {
...
pub static HOLDEM_MAP: LazyLock<FxHashMap<i16, u16>> = LazyLock::new(|| {
    FxHashMap::<i16, u16>::read_from_buffer(&HOLDEM_MAP_DATA).unwrap()
});

pub static SHORTDECK_MAP: LazyLock<FxHashMap<i16, u16>> = LazyLock::new(|| {
    FxHashMap::<i16, u16>::read_from_buffer(&SHORTDECK_MAP_DATA).unwrap()
});
