...
#[test]
fn test_minrank() {
    assert_yes("select count(minrank(boardranks(flop)) = toRank('7')) from board='789'");
}

#[test]
fn test_maxrank() {
    assert_yes("select count(maxrank(boardranks(flop)) = toRank('9')) from board='789'");
}

#[test]
fn test_nthrank() {
    assert_yes("select count(nthrank(1, boardranks(flop)) = toRank('9')) from board='789'");
    assert_yes("select count(nthrank(2, boardranks(flop)) = toRank('8')) from board='789'");
    assert_yes("select count(nthrank(3, boardranks(flop)) = toRank('7')) from board='789'");

    assert_no("select count(nthrank(2, boardranks(flop)) = toRank('7')) from board='AAA'");
}
