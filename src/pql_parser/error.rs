...
pub(super) type LalrError<'input> = ParseError<Loc, Token<'input>, Error>;

pub(super) type ResultE<'input, T> = Result<T, LalrError<'input>>;

pub(super) const fn user_err<'input>(error: Error) -> LalrError<'input> {
...
impl From<&Error> for LocInfo {
    fn from(e: &Error) -> Self {
        match e {
            ...
            | Error::DuplicatedSelectorName(loc) => *loc,
        }
    }
}
