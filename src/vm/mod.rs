...
use crate::{
    ...
    PQLRankSet, PQLStreet, PQLString, PQLType, River, Turn, TypeError, *,
};
...
impl Vm {
    fn sample_next_frame(&mut self) -> Option<()> {
        ...
        self.buffer.flop = (cards[0], cards[1], cards[2]).into();
        self.buffer.turn = cards[3].into();
        self.buffer.river = cards[4].into();
...
mod tests {
    ...
    fn test_sample_next_frame() {
        ...
        assert_eq!(flop!("9cJd7s"), vm.buffer.flop);
    ...
    fn test_sample_try_run() {
        ...
        assert_eq!(Card::C_9S, vm.stack.downcast_pop::<PQLCard>().unwrap());
    }
